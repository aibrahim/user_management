...
The `home-routes` handler in the `user-management.routes.home` namespace
...
The `home-page` function will in turn call the `user-management.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/user_management/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `user-management.handler` namespace:
...
Request middleware functions are located under the `user_management.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `user-management.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `user-management.db.core` in some other namespace.
